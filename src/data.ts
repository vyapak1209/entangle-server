...
export async function createList(
  ...
  await executor(
    `insert into list (id, ownerid, title, lastmodified) values ($1, $2, $3, now())`,
    [list.id, list.ownerID, list.title],
  );
...
export async function getLists(executor: Executor, listIDs: string[]) {
  ...
  const {rows} = await executor(
    `select id, title, ownerid from list where id in (${getPlaceholders(
      listIDs.length,
    )})`,
    listIDs,
  );
...
export async function createTodo(
  ...
  await executor(
    `insert into item (id, listid, title, description, status, priority, ord, lastmodified) values ($1, $2, $3, $4, $5, $6, $7, now())`,
    [todo.id, todo.listID, todo.title, todo.description, todo.status, todo.priority, maxOrd + 1],
  );
  return {
    listIDs: [todo.listID],
    userIDs: [],
...
export async function updateTodo(
  ...
  await requireAccessToList(executor, todo.listID, userID);
  await executor(
    `update item set title = coalesce($1, title), status = coalesce($2, status), description = coalesce($3, description), priority = coalesce($4, priority), ord = coalesce($5, ord), lastmodified = now() where id = $6`,
    [update.title, update.status, update.description, update.priority, update.sort, update.id],
  );
  return {
    listIDs: [todo.listID],
    userIDs: [],
...
export async function getTodos(executor: Executor, todoIDs: string[]) {
  ...
  const {rows} = await executor(
    `select id, listid, title, status, description, priority, ord from item where id in (${getPlaceholders(
      todoIDs.length,
    )})`,
    todoIDs,
  );
