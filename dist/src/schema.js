...
export async function createDatabase(executor) {
...
export async function createSchema(executor) {
    await executor(`drop table if exists replicache_meta, replicache_client_group,
    replicache_client, list, share, item cascade`);
    await executor('create table replicache_meta (key text primary key, value json)');
    await executor("insert into replicache_meta (key, value) values ('schemaVersion', $1)", [schemaVersion]);
...
async function getSchemaVersion(executor) {
