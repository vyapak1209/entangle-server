...
import { createList, createTodo, createShare, deleteList, deleteTodo, deleteShare, putClient, putClientGroup, updateTodo, getClientGroup, getClient, } from './data';
...
export async function push(userID, requestBody) {
...
async function processMutation(userID, clientGroupID, mutation, 
...
errorMode) {
    ...
    return await transact(async (executor) => {
        let affected = { listIDs: [], userIDs: [] };
        console.log('Processing mutation', errorMode ? 'errorMode' : '', JSON.stringify(mutation, null, ''));
...
async function mutate(executor, userID, mutation) {
    switch (mutation.name) {
        ...
        case 'updateTodo':
            return await updateTodo(executor, userID, todoSchema
                .partial()
